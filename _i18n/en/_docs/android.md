...
already. It is technically also possible to use Ainakan without rooting your
device, for example by repackaging the app to include ainakan-gadget, or using a
...
issues like Ainakan crashing the system when launching an app, this is due to
ROM-specific quirks. We cannot test on all possible devices, so we count on
your help to improve on this. However if you're just starting out with Ainakan it
...
First off, download the latest `ainakan-server` for Android from our [releases
page](https://github.com/ainakan/ainakan/releases) and uncompress it.

{% highlight bash %}
$ adb shell getprop ro.product.cpu.abilist # check your device cpu type

$ unxz ainakan-server.xz
{% endhighlight %}
...
$ adb push ainakan-server /data/android/ainakan/
$ adb shell "chmod 755 /data/android/ainakan/ainakan-server"
$ adb shell "/data/android/ainakan/ainakan-server &"
{% endhighlight %}

Some apps might be able to detect the ainakan-server location. Renaming the
ainakan-server binary to a random name, or moving it to another location
such as /dev may do the trick.

For the last step, make sure you start ainakan-server as root, i.e. if you are
...
<div class="note info">
  ...
  <p>
    ...
    <code>adb shell "su -c chmod 755 /data/android/ainakan/ainakan-server"</code>
  </p>
</div>
...
allows Ainakan to discover and communicate with your device regardless of whether
...
{% highlight bash %}
$ ainakan-trace -U -i open -N com.android.chrome
Uploading data...
...
[Messages](/docs/messages), and anywhere you see `ainakan.attach()` just
