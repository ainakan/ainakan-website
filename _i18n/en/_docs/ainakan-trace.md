ainakan-trace is a tool for dynamically tracing function calls.

{% highlight bash %}
# Trace recv* and send* APIs in Safari, insert library names
# in logging
$ ainakan-trace --decorate -i "recv*" -i "send*" Safari

# Trace ObjC method calls in Safari
$ ainakan-trace -m "-[NSView drawRect:]" Safari
...
$ ainakan-trace -U -i "Java_*" com.samsung.faceservice

# Trace a Windows process's calls to "mem*" functions in msvcrt.dll
$ ainakan-trace -p 1372 -i "msvcrt.dll!*mem*"

# Trace all functions matching "*open*" in the process except
# in msvcrt.dll
$ ainakan-trace -p 1372 -i "*open*" -x "msvcrt.dll!*open*"

# Trace an unexported function in libjpeg.so
$ ainakan-trace -p 1372 -a "libjpeg.so!0x4793c"
{% endhighlight %}
...
{% highlight bash %}
$ ainakan-trace -h
usage: ainakan-trace [options] target
...
options:
  ...
  -R, --remote          connect to remote ainakan-server
  -H HOST, --host HOST  connect to remote ainakan-server on HOST
...
This option tells `ainakan-trace` to perform tracing on a remote device
...
ainakan-trace will perform the necessary work to transfer all data to
and from the remote device and trace accordingly.

<div class="note">
  <h5>Copy ainakan-server binary to remote device</h5>
  <p>When tracing a remote device, remember to copy the
  <a href="https://github.com/ainakan/ainakan/releases">platform-appropriate ainakan-server binary</a>
  to the remote device.  Once copied, be sure to run the ainakan-server binary before
  beginning the tracing session.</p>
  <p>For example, to trace a remote Android application, you might copy the
  'ainakan-server-12.8.0-android-arm' binary to the Android's /data/android/ainakan
  folder.  Using adb shell, you would run the server in the background
  (e.g. "ainakan-server-12.8.0-android-arm &").</p>
</div>
...
{% highlight console %}
$ ainakan-trace -p 9753 --decorate -O additional-options.txt
{% endhighlight %}
...
`ainakan-trace` will generate a JavaScript handler file for each function matched
by the `-I` option.
...
<div class="note info">
  <h5>ainakan-trace's working set and the order of inclusions and exclusions</h5>
  <p>ainakan-trace has an internal concept of a "working set", i.e., a set of
...
## -P: Initialize ainakan-trace session with a globally-accessible JSON object
...
<div class="note">
  ...
  <p>
    ...
    Ainakan-trace will assign your JSON object to the global JavaScript variable
...
## -S: Initialize ainakan-trace session with JavaScript code

This option initializes your ainakan-trace session by executing one or more
...
Uses of this powerful feature include initializing the ainakan-trace running
...
[session initialization primer]({% link _docs/ainakan-trace/session-initialization-primer.md %}).

## -d, --decorate: add module name to log tracing

The `--decorate` option is relevant when `ainakan-trace` auto-generates JavaScript
...
function instructs `ainakan-trace` to insert the module name in the default
