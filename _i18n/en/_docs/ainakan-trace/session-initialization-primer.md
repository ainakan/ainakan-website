This page describes uses for the ainakan-trace `--init-session` / `-S` command
...
files during the ainakan-trace engine initialization stage.  These files are
...
your choice is executed before the ainakan-trace engine begins its tracing.
...
The ainakan-trace JavaScript code is often written as one-time "throw-away" code.
...
{% highlight console %}
c:\project> ainakan-trace -p 6980 --decorate -i "gdi32full.dll!ExtTextOutW" -S core.js -S ms-windows.js
Instrumenting...
...
defined when the shared code JavaScript files are executed by ainakan-trace via
the `--init-session` / `-S` command line option.

{% highlight js %}
/*
 * Auto-generated by Ainakan. Please modify to match the signature of ExtTextOutW.
...
Note that, besides calling standard Ainakan functions (e.g., `toInt32()`,
...
meant to be reused by ainakan-trace handlers and shared code libraries.
...
{% highlight js %}
/*
 * Collection of useful general-purpose Ainakan handler functions.
 */
...
/**
 * Produces a somewhat more elegant hex dump, based on Ainakan's own hexdump().
...
{% highlight js %}
/*
 * Collection of useful Ainakan handler functions for MS Windows.
 */
