## Make Android a first-class Ainakan citizen

### Update 2015: This is now implemented in Ainakan thanks to [NowSecure][].

**Brief explanation:** While Ainakan does currently support Android, there
are two missing pieces that result in a lot of friction when instrumenting
Android apps, and we need to improve on those:

- Packaging: Package up ainakan-server and either run it as a system
daemon or bundle a launcher app.

- Integration: Add an Android backend for automating USB device discovery
and port forwarding. This is similar to Ainakan's iOS «Fruity» backend that
...
**Brief explanation:** Ainakan currently supports Dalvik, and while most of
that code is just interacting with the JNI APIs implemented by the VM, there
are some bits that are VM-specific. The current code can be found [here](https://github.com/ainakan/ainakan-gum/blob/42b69917976f43ba3ec4297046b319970dc037dd/gum/gumscript-runtime-dalvik.js).
...
deprecated alias kept around until the next major Ainakan release.
...
**Brief explanation:** Ainakan's Stalker is a very powerful code tracing engine
based on dynamic recompilation. It is currently only available for x86. Porting
it to ARM would allow [CryptoShark](https://github.com/ainakan/cryptoshark) to be
used on mobile apps.
...
**Brief explanation:** Ainakan's Stalker is a very powerful code tracing engine
based on dynamic recompilation. It is currently only available for x86. Porting
it to ARM64 would allow [CryptoShark](https://github.com/ainakan/cryptoshark) to
...
which is already present in Ainakan, this is about adding support for
...
and also improve the visibility of Ainakan by being present in as many ecosystems
as possible.

**Expected results:** Packages for major distributions automatically published
by Ainakan's buildbot.
...
## Port Ainakan to Windows Phone

**Brief explanation:** Ainakan currently supports Windows, macOS, Linux, iOS,
Android, and QNX, but sadly not yet Windows Phone. Adding support for WP would
require:

- An injector to get Ainakan's shared library injected into the target process

- Process spawn support

- JavaScript runtime that interacts with the CLR runtime
dynamically, similar to the [Dalvik JS runtime](https://github.com/ainakan/ainakan-gum/blob/42b69917976f43ba3ec4297046b319970dc037dd/gum/gumscript-runtime-dalvik.js)
that's built into Ainakan's JS environment.
...
**Brief explanation:** There's currently a [Backtracer](https://github.com/ainakan/ainakan-gum/blob/42b69917976f43ba3ec4297046b319970dc037dd/gum/gumbacktracer.h)
and a [symbol resolving](https://github.com/ainakan/ainakan-gum/blob/42b69917976f43ba3ec4297046b319970dc037dd/gum/gumsymbolutil.h)
API in ainakan-gum that are not yet exposed to the JS runtime.
