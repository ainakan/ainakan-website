...
adds `ainakan-ps` for enumerating processes both locally and remotely.
...
The `-U` switch is also accepted by `ainakan-trace` and `ainakan-discover`.
...
Ainakan got its first contribution since the HN launch.
[Pete Morici](https://github.com/pmorici) dived in and contributed support
for specifying module-relative functions in `ainakan-trace`:

{% highlight bash %}
$ ainakan-trace -a 'kernel32.dll+0x1234'
{% endhighlight %}
