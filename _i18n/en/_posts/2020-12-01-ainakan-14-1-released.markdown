...
With these improvements we will finally support building past versions of Ainakan
...
{% highlight bash %}
$ make -f Makefile.sdk.mk AINAKAN_HOST=android-arm64 libunwind
{% endhighlight %}
...
But you might already have built Ainakan, and want to switch out libunwind in the
...
no elevation is needed – we can now avoid writing out “ainakan-helper-{32,64}.exe”
...
using “ainakan-compile -b” unless your application is designed to only be used
with one exact version of Ainakan.

As I wasn't aware of this pitfall when cutting the previous release of
ainakan-tools, I opted to precompile the ainakan-trace agent to bytecode. Upon
realizing my mistake while working on releasing 14.1, I reverted this mistake
and released a new version of ainakan-tools.

So make sure you also grab its latest release while upgrading:

{% highlight bash %}
$ pip3 install -U ainakan-tools
{% endhighlight %}
...
- Heavily refurbished build system for dependencies. Going forward we will
  finally support building past versions of Ainakan fully from source.
...
- Add --ignore-crashes/-C to ainakan-server for disabling the native crash
