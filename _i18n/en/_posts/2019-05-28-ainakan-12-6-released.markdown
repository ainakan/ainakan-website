...
So bottom line is that this is the most stable version of Ainakan ever
released – and now is the time to make sure you're running Ainakan 12.6.
...
- Ainakan can now talk to old versions of Ainakan, from before the addition
...
- The *ainakan-inject* tool now supports passing parameters to the script through
...
- Child-gating is more reliable as Ainakan now allocates file-descriptors in a
...
- Ctrl+C is handled by ainakan-server on Windows to support graceful shutdown
...
- The *ainakan-gadget-ios* meta-package is now dependency-free.
...
- Ainakan no longer crashes on i/macOS if the target process dies while injecting
  a library into it.
...
- Ainakan no longer crashes when logging early in the process lifetime on iOS.
...
- The *ainakan-inject* CLI tool was improved to exit on detach.
...
- Ainakan no longer crashes when *HostSession* gets lost mid-request.
