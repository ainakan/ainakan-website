...
but it wasn't until Ainakan 10.5 in late 2017 that we [started][] unleashing its
massive potential.
...
automatically exclude ainakan-agent's own memory range, and that way we didn't
...
ainakan-agent's code is marked as excluded.
...
which [ainakan-fuzz][] is a great example of. There's also a bunch of other
...
using an existing module such as [ainakan-fs][] where you get an ArrayBuffer that
...
- The Python bindings allow obtaining a file-descriptor from a Cancellable in
  ...
  Worth noting that ainakan-tools 7.0.1 is out with a major improvement built on
  this: The CLI tools no longer delay for up to 500 ms before exiting. So
  short-lived programs like *ainakan-ls-devices* and *ainakan-ps* now feel very
  snappy.
...
- Improved Android fault tolerance, handling *zygote* and *system_server*
  processes dying without having to restart *ainakan-server*.
- Now able to launch *ainakan-server* during boot on Android 10, as
  *LD_LIBRARY_PATH* no longer interferes with the spawning of *ainakan-helper-32*.
...
[started]: /news/2017/08/25/ainakan-10-5-released/
[AirSpy]: https://github.com/nowsecure/airspy
[ainakan-fuzz]: https://twitter.com/andreafioraldi/status/1205194910372110337
[@andreafioraldi]: https://twitter.com/andreafioraldi
[ainakan-fs]: https://github.com/nowsecure/ainakan-fs
