...
- Rely on a device-side ainakan-server to figure things out for us: This wouldn't
...
ainakan-server and ainakan-gadget so they listen on tunnel interfaces whenever they
appear.
...
Given that Ainakan needs to speak quite a few protocols to interact with Apple's
...
wasting time establishing a connection that Ainakan already established for its
own needs.
...
quite a few, and would result in the Ainakan API becoming massive. It would also
make Ainakan a kitchen sink of sorts, and that's clearly not a direction we want
to be heading in.
...
{% highlight js %}
import ainakan from 'ainakan';
import util from 'util';

const device = await ainakan.getUsbDevice();
      ...
      char * argv[])
{
  ...
  AinakanDeviceManager * manager = ainakan_device_manager_new ();

  AinakanDevice * device = ainakan_device_manager_get_device_by_type_sync (manager, AINAKAN_DEVICE_TYPE_USB, -1, cancellable, &error);
  AinakanService * service = ainakan_device_open_service_sync (device, "xpc:com.apple.coredevice.appservice", cancellable, &error);
  ...
  "}");
  GVariant * response = ainakan_service_request_sync (service, parameters, cancellable, &error);
...
whenever a notification is received. Since Ainakan's APIs also offer synchronous
...
[kernel driver patch]: https://lore.kernel.org/all/20231130220109.90734-2-oleavr@ainakan.re/T/#ma666f28da8f071f3433e915a28b2152d08373de5
