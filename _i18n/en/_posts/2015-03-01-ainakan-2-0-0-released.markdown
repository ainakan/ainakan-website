...
- Mac and iOS injector performs manual mapping of Ainakan's dylib. This means
  we're able to attach to heavily sandboxed processes.
- The CLI tools like *ainakan-trace*, *ainakan-repl*, etc., have brand new support
  for spawning processes:
{% highlight bash %}
$ ainakan-trace -i 'open*' -i 'read*' /bin/cat /etc/resolv.conf
    27 ms	open$NOCANCEL()
    28 ms	read$NOCANCEL()
    28 ms	read$NOCANCEL()
    28 ms	read$NOCANCEL()
...
$
{% endhighlight %}
- Usability improvements in *ainakan-repl* and *ainakan-discover*.
...
- The python API now provides you with `ainakan.get_usb_device(timeout = 0)` and
  `ainakan.get_remote_device()` for easy access to iOS and remote/Android
  devices.
...
Want to help out? Have a look at our [GSoC 2015 Ideas Page](https://ainakan.re/docs/gsoc-ideas-2015/)
...
bug when using Ainakan on Windows with certain iOS device configurations.
