...
First off is the long-standing issue where multiple Ainakan clients attached to
...
This was probably not a big deal for most users of Ainakan. However, we also had
the same issue if one running *ainakan-server* was shared by multiple clients.
You might have *ainakan-trace* running in one terminal while using the REPL in
...
was due to some crazy logic in ainakan-server that would keep track of how many
...
Ainakan will unload its agent from the target process.

That was the big change of this release, but we didn't stop there.

One important feature of Ainakan's scripts is that you can exchange messages with
