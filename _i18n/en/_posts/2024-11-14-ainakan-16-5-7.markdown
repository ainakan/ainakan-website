...
- **Added support for the TCP tunnel protocol** and made it the default to match
  Apple's new behavior. The `AINAKAN_FRUITY_TUNNEL_PROTOCOL` environment variable
...
- **Added support for binary data handling** in `ainakan-inject` and RpcClient
  internals.
