...
OSes. Those of you using Ainakan on i/macOS may have noticed that spawn() stopped
...
if found vectors execution there instead. Explaining why this broke Ainakan's
spawn() functionality needs a little context though, so bear with me.

Part of what Ainakan does when you call attach() is to inject its agent, if it
...
at dyld's entrypoint, Ainakan basically advances the main thread's execution until
...
dyld_shared_cache, Ainakan was placing a breakpoint in the version mapped in from
...
The ainakan.Compiler just got a whole lot better, and now supports additional
configuration through tsconfig.json, as well as using local ainakan-gum typings.
...
## Cross-post: ainakan-tools 12.0.2

We also have a brand new release of ainakan-tools, which thanks to [@tmm1][] has a
new and exciting feature. The *ainakan-ls-devices* tool now displays higher
fidelity device names, with OS name and version displayed in a fourth column:

![ainakan-ls-devices](/img/ls-devices-12-0-2.png "ainakan-ls-devices in 12.0.2"){: width="100%" }

To upgrade:

{% highlight bash %}
$ pip3 install -U ainakan ainakan-tools
{% endhighlight %}
...
- compiler: Fix support for local ainakan-gum typings.
- compiler: Use latest *@types/ainakan-gum* from git.
...
- devkit: Add .gir to the ainakan-core kit. Thanks [@lateralusd][]!
...
[fix]: https://github.com/ainakan/ainakan-core/commit/73ac5eab9e6912bbd9903270e629e0d0ca773209
