...
would previously take a while to start when launched using Ainakan should now be a
lot quicker to start. This long-standing bottleneck was so bad that an app with
a lot of libraries could fail to launch due to Ainakan slowing down its startup
too much.
...
sometimes end up in a situation where either Ainakan or the target process
