---
layout: news_item
title: 'Ainakan 15.0 Released'
date: 2021-07-18 00:48:05 +0200
author: oleavr
version: 15.0
categories: [release]
redirect_from: /news/2021/06/16/ainakan-15-0-released/index.html
---
...
distributed instrumentation use-cases. Essentially ship a Ainakan Gadget that's
...
code, and existing tools such as ainakan-trace won't actually be usable in such a
setup.

That's when @insitusec suggested that perhaps Ainakan's Gadget could offer an
inverse counterpart to its [Listen][] interaction. So instead of it being a
server that exposes a ainakan-server compatible interface, it could be configured
...
ainakan-server compatible interface where all of them appear as processes. To the
...
enumerate_processes() or ainakan-ps, and one can attach() to them seamlessly.

In this way, existing Ainakan tools work exactly the same way – and by enabling
...
2. Optionally also a ***control*** interface that controllers can talk to. E.g.
   `ainakan-trace -H my.portal.com -n Twitter -i open`

To a user this would be pretty simple: just grab the ainakan-portal binary from
our [releases][], and run it on some machine that the Gadget is able to
[reach][]. Then point tools at that – as if it was a regular ainakan-server.

That is however only one part of the story – how it would be used for simple
use-cases. The ainakan-portal CLI program is actually nothing more than a thin CLI
...
One can also use our ainakan-core language bindings, for e.g. Python or Node.js,
...
standard Ainakan Device object, on which one can enumerate_processes(), attach(),
...
Given how useful it might be to run a ainakan-portal on the public Internet, it
...
manager = ainakan.get_device_manager()
...
ainakan-portal on the public Internet, is authentication. In this case our server
...
cluster_params = ainakan.EndpointParameters(authentication=('token', "wow-such-secret"))
control_params = ainakan.EndpointParameters(authentication=('callback', authenticate))
...
Norway using ainakan-trace. It would be a shame if the script messages coming from
...
{% highlight sh %}
$ ainakan-join -U ReversingTarget my.portal.com cert.pem secret
{% endhighlight %}

Now I can jump in with the Ainakan REPL:
...
{% highlight python %}
session = ainakan.get_usb_device().attach("ReversingTarget")
...
Something I've been wanting to build since before Ainakan was born, is an online
collaborative reversing app. Back in the very beginning of Ainakan, I built a
...
of Ainakan, and that's super-exciting! But what I've noticed when toying with SPAs
on my own, is that it's quite tedious to have to write the middleware.

Well, with Ainakan 15 I had to make some protocol changes to accomodate the
...
async function start() {
  ...
  const hostSessionObj = await bus.getProxyObject('re.ainakan.HostSession15',
      '/re/ainakan/HostSession');
...
This means that Ainakan's network protocol is now WebSocket-based, so browsers can
finally talk directly to a running Portal/ainakan-server, without any middleware
...
{% highlight sh %}
$ ./ainakan-portal --asset-root=/path/to/web/app
{% endhighlight %}

This is also easy at the API level:

{% highlight python %}
control_params = ainakan.EndpointParameters(asset_root="/path/to/web/app")
service = ainakan.PortalService(cluster_params, control_params)
{% endhighlight %}
...
that don't make use of the Bus, such as ainakan-trace.)
...
order to run code inside Ainakan itself (e.g. in a remote ainakan-server), it is
...
{% highlight sh %}
$ python3 -c 'import ainakan; import json; \
    print(json.dumps(ainakan.query_system_parameters()))' \
    | jq
{% endhighlight %}
...
a remote ainakan-server – which is what `access: 'full'` means.
...
make use of it. So now with Ainakan 15 you might notice that application and
...
provide in the rooted case, where we have a ainakan-server running on the Android
side.
...
For those of you using Ainakan on Android, you may have encountered apps where
...
ainakan-java-bridge to support Android 12.
...
“just works” – you should see them if you run `ainakan-ls-devices`.
...
So if you're using `-U` or `ainakan.get_usb_device()` things will work just like
...
The second pitfall is that ainakan-server only listens on the loopback interface
...
edit `/Library/LaunchDaemons/re.ainakan.server.plist` to add the `--listen`
...
- Introduce PortalService API and daemon, a network service that
  orchestrates a cluster of remote processes instrumented by Ainakan.
  Implements both a ainakan-server compatible control interface, as well
...
- Add Session.join_portal(), making it easy to share control of a
  ...
  other nodes.
- Add “connect” interaction to ainakan-gadget, so it can join a
  PortalService cluster as well.
- Add PortalClient, used to implement Session.join_portal() and
  ainakan-gadget's “connect” interaction. Connects to the PortalService
...
- Improve jailed Android injector: Now using a long-lived shell session to speed
  things up. Also ensure filenames dropped into /data/android/ainakan are unique, and
...
- Lazily create the ScriptScheduler thread pool. This means anyone using GumJS
  ...
  implement logic to stop and later restart threads. (Something ainakan-agent
...
- Implement a new ainakan-pipe strategy for i/macOS. Turns out that our previous
...
- Linux: Use an abstract name for ainakan-pipe when supported.
- Unlink UNIX socket when ainakan-pipe is done with it.
...
- web-service: Implement basic directory listing with NGINX-style output. This
  is useful with e.g. “ainakan-server --asset-root=/”.
...
[Listen]: https://ainakan.re/docs/gadget/#listen
[crystallized]: https://github.com/ainakan/ainakan-core/blob/0120cf59bc6f623bb842c93bc8870ce2a704453c/src/portal-service.vala
[releases]: https://github.com/ainakan/ainakan/releases
[reach]: https://ainakan.re/docs/gadget/#connect
[200 lines of code]: https://github.com/ainakan/ainakan-core/blob/0120cf59bc6f623bb842c93bc8870ce2a704453c/portal/portal.vala
[glib-networking]: https://gitlab.gnome.org/GNOME/glib-networking
[client]: https://github.com/ainakan/ainakan-core/blob/0120cf59bc6f623bb842c93bc8870ce2a704453c/src/socket/socket-host-session.vala#L117-L133
[PortalClient]: https://github.com/ainakan/ainakan-core/blob/0120cf59bc6f623bb842c93bc8870ce2a704453c/lib/payload/portal-client.vala
[here]: https://github.com/ainakan/ainakan-python/blob/bbdcdfaca821276361399775bd81599710ec0625/examples/session_persist_timeout.py
[libnice]: https://libnice.freedesktop.org/
[GLib]: https://gitlab.gnome.org/GNOME/glib
[mode]: https://ainakan.re/docs/modes/
[examples/web_client]: https://github.com/ainakan/ainakan-python/blob/bbdcdfaca821276361399775bd81599710ec0625/examples/web_client/src/store/plugins/ainakan.ts
[peer-to-peer implementation]: https://github.com/ainakan/ainakan-core/blob/0120cf59bc6f623bb842c93bc8870ce2a704453c/lib/base/p2p.vala
[examples/portal_server.py]: https://github.com/ainakan/ainakan-python/blob/bbdcdfaca821276361399775bd81599710ec0625/examples/portal_server.py
[examples/portal_client.py]: https://github.com/ainakan/ainakan-python/blob/bbdcdfaca821276361399775bd81599710ec0625/examples/portal_client.py
