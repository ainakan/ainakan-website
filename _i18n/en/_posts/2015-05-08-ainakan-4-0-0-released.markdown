...
Let's start with a user-facing change. The CLI tool called *ainakan-repl* has
been renamed to just *ainakan*, and now does tab completion! This and some other
...
{% highlight sh %}
$ ainakan Calculator -l calc.js
    _____
   (_____)
    |   |    Ainakan 4.0.0 - A world-class dynamic
...
Ainakan just got an integrated Node.js-compatible debugger.
...
{% highlight bash %}
# Connect Ainakan to a locally-running Calculator.app
# and load calc.js with the debugger enabled
$ ainakan Calculator -l calc.js --debug
    _____
   (_____)
    |   |    Ainakan 4.0.0 - A world-class dynamic
    |   |                  instrumentation framework
...
![Ainakan Debugger Session](/img/ainakan-debug.png "Ainakan Debugger Session")

Ever found yourself wanting to *ainakan-trace* Objective-C APIs straight from
...
necessary to forward multiple TCP ports when using `ainakan-server` running
...
{% highlight python %}
ainakan.ProcessNotFoundError: unable to find process with pid 1234
{% endhighlight %}

That's better. Let's talk about performance. Perhaps you used ainakan-trace and
...
- `Ainakan.version` can be interrogated to check which version you're running,
  and this is also provided on the *ainakan-core* end, which for example is
  exposed by *ainakan-python* through `ainakan.__version__`.
- *Stalker* now supports the *jecxz* and *jrcxz* instructions. This is good news
  for [CryptoShark](https://github.com/ainakan/cryptoshark), which should soon
  provide some updated binaries to bundle the latest version of Ainakan.
