...
ABI-stable since version 5, this long-awaited bridge allows Ainakan to play nicely
...
So to dump types, Ainakan basically iterates over these data structures and
...
Ainakan also has the advantage of being able to probe into
...
- Implement the Swift bridge, which allows Ainakan to:
...
- portal-service: Always remove ClusterNode sessions whenever a session ID is
  ...
  Thanks [@mrmacete][]!
- ainakan-portal: Fix typo in --help output.
...
- agent: Fix hang on unload w/ NO_REPLY_EXPECTED calls, where we would wait for
  ...
  Vala compiler, and it previously (in Ainakan <= 15.1.9) ignoring
...
[documentation]: https://github.com/ainakan/ainakan-swift-bridge/blob/master/docs/api.md
