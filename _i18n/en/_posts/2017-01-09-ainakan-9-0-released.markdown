...
Let's talk about Duktape first. Ainakan's first JS runtime was based on [V8][],
...
JS transitions, so if your Ainakan agent is all about API hooks, and your hooks
...
non-trivial agents should be using [ainakan-compile][], which compiles your code
to ES5.
...
where Ainakan and Substrate would both hook *posix_spawn()* in launchd and
...
launching. But still, it was the only part of Ainakan that indirectly required
...
{% highlight bash %}
$ ainakan-trace -U -f com.atebits.Tweetie2 -i open
{% endhighlight %}

We're telling it to launch Twitter's iOS app and trace functions named *open*.
As a side-note, if you're curious about the details, ainakan-trace is written in
Python and is less than 900 lines of [code][], so it might be a good way to
learn more about building your own tools on top of Ainakan. Or perhaps you'd
like to improve ainakan-trace? Even better!
...
suspended at dyld's entrypoint. ainakan-trace will then want to attach to it
...
Now that it has applied the instrumentation, it will ask Ainakan to resume
...
much disk space is required when Ainakan is installed on an iOS device and is
...
- We used to write the ainakan-helper binary out to a temporary file and spawn it.
  The meat of the ainakan-helper program is now statically linked into
  ainakan-server, and its entitlements have been boosted along with it. This
  binary is only necessary when Ainakan is used as a plugin in an unknown process,
  i.e. where we cannot make any guarantees about entitlements and code-signing.
  In the ainakan-server case, however, it is able to guarantee that all such
  constraints are met.
- The library that we inject into processes to be instrumented,
  *ainakan-agent.dylib*, is no longer written out to a temporary file. We use
  our own out-of-process dynamic linker to map it from ainakan-server's memory
...
- V8 was disabled for the iOS binaries as it's only really usable on old
  ...
  (If V8 is important to your use-case, you can build it like this:
   `make server-ios AINAKAN_DIET=no`)
- The iOS package has been split into two, “Ainakan” for 64-bit devices, and
  “Ainakan for 32-bit devices” for old devices.
- Getting rid of the Substrate dependency for iOS app launching also meant
  we got rid of AinakanLoader.dylib. This is however a very minor improvement.
...
[ainakan-compile]: https://github.com/ainakan/ainakan-compile
[launchd.js]: https://github.com/ainakan/ainakan-core/blob/12be27ab171c8bac9b4a60db5b3957f30f4be938/src/darwin/agent/launchd.js
[prepareForLaunch()]: https://github.com/ainakan/ainakan-core/blob/12be27ab171c8bac9b4a60db5b3957f30f4be938/src/darwin/agent/launchd.js#L18-L21
[SBSLaunchApplicationWithIdentifierAndLaunchOptions()]: https://github.com/ainakan/ainakan-core/blob/12be27ab171c8bac9b4a60db5b3957f30f4be938/src/darwin/ainakan-helper-backend-glue.m#L560-L563
[POSIX_SPAWN_START_SUSPENDED]: https://github.com/ainakan/ainakan-core/blob/12be27ab171c8bac9b4a60db5b3957f30f4be938/src/darwin/agent/launchd.js#L60
[signals back]: https://github.com/ainakan/ainakan-core/blob/12be27ab171c8bac9b4a60db5b3957f30f4be938/src/darwin/agent/launchd.js#L85
[xpcproxy.js]: https://github.com/ainakan/ainakan-core/blob/12be27ab171c8bac9b4a60db5b3957f30f4be938/src/darwin/agent/xpcproxy.js
[wait for the exec]: https://github.com/ainakan/ainakan-core/blob/12be27ab171c8bac9b4a60db5b3957f30f4be938/src/darwin/darwin-host-session.vala#L476-L478
[code]: https://github.com/ainakan/ainakan-python/blob/9c876f457cdee4d3dab6c05c8ab8c4bd72ca42d1/src/ainakan/tracer.py
[here]: https://github.com/ainakan/ainakan-core/blob/12be27ab171c8bac9b4a60db5b3957f30f4be938/src/darwin/ainakan-helper-backend-glue.m#L835-L861
