...
bugs and deadlocks when using Ainakan on iOS 9. The root cause was simply that our
inline hooking was causing the process to lose the *CS_VALID* bit of its
[code-signing status](https://github.com/ainakan/ainakan-gum/blob/ae22e0fa94970a9df140757e4aa0467e9deea9aa/tests/core/interceptor.c#L1111).
...
containing *open()*, then pseudo-sign this binary, ask the kernel to [F_ADDFILESIGS](https://github.com/ainakan/ainakan-gum/blob/ae22e0fa94970a9df140757e4aa0467e9deea9aa/gum/backend-darwin/gumcodesegment-darwin.c#L211)
...
Starting with this release, our Interceptor API now supports [transactions](https://github.com/ainakan/ainakan-gum/blob/ae22e0fa94970a9df140757e4aa0467e9deea9aa/gum/guminterceptor.h#L78-L79).
...
used *ainakan-trace* to trace Objective-C APIs, or glob for functions across all
...
could even take so long that we exceeded the system's [launch timeout](https://github.com/ainakan/ainakan/issues/103).
...
*ainakan-trace* does, we now have a [brand new API](https://ainakan.re/docs/javascript-api/#apiresolver)
for just that:
...
- ainakan-trace: improve performance by removing *setTimeout()* logic, allowing
               many hooks to be applied in the same transaction
- ainakan-trace: batch log events in 50 ms chunks to improve performance

6.2.1:

- core: add *ApiResolver* API
- ainakan-trace: improve performance by using the new *ApiResolver* API

6.2.2:

- core: fix oops that prevented injection into Windows Store/Universal apps
- core: fix crash on teardown on 32-bit ARM
- core: add ainakan-inject, a tool to inject an agent into a running process with
        similar semantics to ainakan-gadget
...
- core: fix source-map handling for eval code, which manifested itself as
        unhandled exceptions getting swallowed, e.g. when running ainakan-trace
- core: fix Python 3.x build system regression
- ainakan-trace: fix path escaping issue
- ainakan-trace: improve error-handling for bad handlers

6.2.4:

- ainakan-trace: monitor handlers instead of polling them
...
- ainakan-repl: simplify logic by using the RPC feature
...
- core: fix the ainakan-inject build system on Mac
- core: (Windows) fix crash when USB device location retrieval fails
- ainakan-server: allow overriding the default listen address
- ainakan-node: add *addRemoteDevice()* and *removeRemoteDevice()* to
              DeviceManager
- ainakan-python: add -H switch for specifying the host to connect to
- ainakan-python: add *add_remote_device()* and *remove_remote_device()* to
                DeviceManager
- ainakan-python: fix compatibility issues with the Duktape runtime
- ainakan-python: canonicalize the requested RPC method name
