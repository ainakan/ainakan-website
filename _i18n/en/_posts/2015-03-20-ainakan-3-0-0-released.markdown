...
We just brought you brand new [Node.js bindings](https://github.com/ainakan/ainakan-node),
and they are fully asynchronous:

{% gist 6ecae99945ccba47427a %}

Check out the [examples](https://github.com/ainakan/ainakan-node/blob/46a5f92203ab86978a2af68d6c926d6d2b63fbe7/examples/interactive.js)
...
Ainakan. By offering reverse-engineering and software development expertise,
the goal is to generate enough revenue to pay my bills and leave some time
to work on Ainakan. Longer term I'm hoping there will also be demand for help
adding features or integrating Ainakan into third-party products.
...
64-bit Ainakan process, e.g. your Python interpreter, can inject into a 32-bit
process. The other direction works too.

Another awesome update is that [Tyilo](https://github.com/Tyilo) contributed
[improvements](https://github.com/ainakan/ainakan-python/commit/daf1a310670588e5672af2205658598be342c2e2)
to *ainakan-trace* so it now uses man-pages for auto-generating the log handlers.
Awesome, huh? But there's even more goodies:

- *ainakan-server* ports are now recycled, so if you're using Ainakan on Android
...
- Python bindings have a minor simplification so instead of
  `ainakan.attach(pid).session.create_script()` it's simply just
  `ainakan.attach(pid).create_script()`. This is just like in the brand
