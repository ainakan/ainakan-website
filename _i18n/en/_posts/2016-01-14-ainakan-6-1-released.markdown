...
Some time ago [@s1341](https://github.com/s1341) ported Ainakan to QNX, and just a
few weeks back he was running into memory footprint issues when using Ainakan on
embedded ARM devices. This was right after he contributed pull-requests porting
Ainakan to linux-arm. We started realizing that it might be time for a new
...
like V8 and a pure interpreter might not really matter for most users of Ainakan.
...
- ainakan-repl: upgrade to latest prompt-toolkit
- ainakan-trace: fix crash when attempting to trace partially resolved imports
- ainakan-trace: stick to ES5 in the generated handlers for Duktape compatibility
...
- core: give the iOS ainakan-helper all the entitlements that the Preferences app
...
- core: fix mapper issues preventing us from injecting into Mac processes
        where libraries ainakan-agent depends are not yet loaded
- core: make InvocationContext available to replaced functions also

6.1.5:

- core: add support for generator functions in scripts generated by ainakan-load
- ainakan-repl: fix race condition resulting in hang
- ainakan-repl: fix spurious error message on exit
