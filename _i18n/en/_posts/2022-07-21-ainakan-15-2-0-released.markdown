...
streamline Ainakan's JavaScript developer experience. As a developer I may start
...
to use some off-the-shelf packages from npm, such as [ainakan-remote-stream][].
...
[@ainakan/rollup-plugin-node-polyfills][] for interop with packages from npm, and
...
created [ainakan-compile][] as a simple tool that does the plumbing for you, with
configuration defaults optimized for what makes sense in a Ainakan context. Still
though, this does require some boilerplate such as package.json, tsconfig.json,
and so forth.

To solve that, I published [ainakan-agent-example][], a repo that can be cloned
and used as a starting point. That is still a bit of friction, so later
ainakan-tools got a new CLI tool called ainakan-create. Anyway, even with all of
...
Then it struck me. What if we could use ainakan-compile to compile ainakan-compile
into a self-contained .js that we can run on Ainakan's system session? The system
session is a somewhat obscure feature where you can load scripts inside of the
process hosting ainakan-core. For example if you're using our Python bindings,
that process would be the Python interpreter.

Once we are able to run that ainakan-compile agent inside of GumJS, we can
communicate with it and turn that into an API. This API can then be exposed
by language bindings, and ainakan-tools can consume it to give the user a
ainakan-compile CLI tool that doesn't require Node.js/npm to be installed. Tools
...
Traceback (most recent call last):
  File "/home/oleavr/src/explore.py", line 4, in <module>
    bundle = compiler.build("agent.ts")
  File "/home/oleavr/.local/lib/python3.10/site-packages/ainakan/core.py", line 76, in wrapper
    return f(*args, **kwargs)
  File "/home/oleavr/.local/lib/python3.10/site-packages/ainakan/core.py", line 1150, in build
    return self._impl.build(entrypoint, **kwargs)
ainakan.NotSupportedError: compilation failed
{% endhighlight %}
...
{% highlight js %}
console.log("Hello from Ainakan:", Ainakan.version);
{% endhighlight %}
...
✄
console.log(`Hello ${Ainakan.version}!`);
{% endhighlight %}
...
{% highlight bash %}
$ ainakan -p 0 -l _agent.js
     ____
    / _  |   Ainakan 15.2.0 - A world-class dynamic instrumentation toolkit
   | (_| |
    > _  |   Commands:
...
log("Hello from Ainakan:", Ainakan.version);
{% endhighlight %}
...
Loading bundle...
Hello from Ainakan version: 15.2.0
{% endhighlight %}
...
We can also use ainakan-tools' new ainakan-compile CLI tool:

{% highlight bash %}
$ ainakan-compile agent.ts -o _agent.js
{% endhighlight %}

It also supports watch mode:

{% highlight bash %}
$ ainakan-compile agent.ts -o _agent.js -w
{% endhighlight %}

## REPL

Our REPL is also powered by the new ainakan.Compiler:

{% highlight bash %}
$ ainakan -p 0 -l agent.ts
     ____
    / _  |   Ainakan 15.2.0 - A world-class dynamic instrumentation toolkit
   | (_| |
    > _  |   Commands:
...
where we were working on ainakan.Compiler together! 🙌
...
[ainakan-remote-stream]: https://github.com/nowsecure/ainakan-remote-stream
[Rollup]: https://rollupjs.org/guide/en/
[@ainakan/rollup-plugin-node-polyfills]: https://www.npmjs.com/package/@ainakan/rollup-plugin-node-polyfills
[@rollup/plugin-typescript]: https://www.npmjs.com/package/@rollup/plugin-typescript
[ainakan-compile]: https://www.npmjs.com/package/ainakan-compile
[ainakan-agent-example]: https://github.com/oleavr/ainakan-agent-example
