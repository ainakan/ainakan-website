---
layout: news_item
title: 'Ainakan 1.0.5 Released'
date: 2014-01-05 23:00:00 +0100
author: oleavr
version: 1.0.5
categories: [release]
---

This release improves [ainakan-trace][] with support for auto-generating,
loading and reloading function handlers from scripts on the filesystem.
Have a look at our [Quick-start guide](/docs/quickstart) for a walkthrough.
Also new in this release is Py3k support, which is available from [PyPI][] on
all platforms.

[ainakan-trace]: https://github.com/ainakan/ainakan-python/blob/master/src/ainakan/tracer.py
[PyPI]: https://pypi.python.org/pypi/ainakan
