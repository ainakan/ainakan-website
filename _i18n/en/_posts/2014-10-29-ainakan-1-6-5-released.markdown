...
- The iOS USB transport no longer disconnects when sending a burst of data to
  the device. This would typically happen when using `ainakan-trace` and tracing
  ...
  but was very reproducible when using Ainakan with a tethered iOS device.
- Eliminated crashes on shutdown of the Python interpreter.
- The `onEnter` and `onLeave` callbacks in `ainakan-trace` scripts are now called
