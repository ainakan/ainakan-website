...
For quite some years I've been dreaming of taking Ainakan beyond user-space
...
The idea I had been thinking about for a long time was to add a new Ainakan
...
better to talk to its GDB-compatible remote stub. In this way, Ainakan will be
...
Using the Ainakan REPL:

{% highlight bash %}
$ ainakan -D barebone -p 0 -l demo.js
     ____
    / _  |   Ainakan 16.1.0 - A world-class dynamic instrumentation toolkit
   | (_| |
    > _  |   Commands:
...
- We set the least significant bit to indicate to Interceptor that the target
  ...
  you if you have previously used Ainakan's conventional backends on 32-bit ARM.
- The new Barebone backend connects to the GDB-compatible remote stub at
  *127.0.0.1:3333* by default. This matches what OpenOCD typically defaults to,
  but can be overridden by setting the *AINAKAN_BAREBONE_ADDRESS* environment
  variable.
...
Next, we'll use the Ainakan REPL to look around:

{% highlight bash %}
$ export AINAKAN_BAREBONE_ADDRESS=127.0.0.1:9000
$ ainakan -D barebone -p 0
     ____
    / _  |   Ainakan 16.1.0 - A world-class dynamic instrumentation toolkit
   | (_| |
    > _  |   Commands:
...
{% highlight bash %}
$ ainakan -D barebone -p 0 -l scan.js
     ____
    / _  |   Ainakan 16.1.0 - A world-class dynamic instrumentation toolkit
   | (_| |
    > _  |   Commands:
...
{% highlight bash %}
$ ainakan -D barebone -p 0 -l kernhook.js
     ____
    / _  |   Ainakan 16.1.0 - A world-class dynamic instrumentation toolkit
   | (_| |
    > _  |   Commands:
...
{% highlight bash %}
$ ainakan -D barebone -p 0 -l kernhook2.js
     ____
    / _  |   Ainakan 16.1.0 - A world-class dynamic instrumentation toolkit
   | (_| |
    > _  |   Commands:
...
Error: to enable this feature, set AINAKAN_BAREBONE_HEAP_BASE to the physical base address to use, e.g. 0x48000000
    at <eval> (/home/oleavr/src/demo/kernhook2.js:13)
    at evaluate (native)
    at <anonymous> (/ainakan/repl-2.js:1)
...
*enumerate_processes()*, so ainakan-ps works. And those are only a couple of
examples... How about injecting ainakan-gadget into a user-space process? That
would be super-useful for an embedded system where we want to avoid modifying
the flash. Anyway, I digress 😊

So, on MCUs and unknown kernels you will have to tell Ainakan where, in physical
...
*AINAKAN_BAREBONE_HEAP_BASE* environment variable:

{% highlight bash %}
$ export AINAKAN_BAREBONE_HEAP_BASE=0x48000000
$ ainakan -D barebone -p 0 -l kernhook2.js
     ____
    / _  |   Ainakan 16.1.0 - A world-class dynamic instrumentation toolkit
   | (_| |
...
Corellium as well. All you need to do is point *AINAKAN_BAREBONE_ADDRESS* at the
...
And as a fun aside, here's Ainakan attached to the BeOS kernel:
...
- darwin: Improve tvOS support to also cover ainakan-server. Thanks [@tmm1][]!
- darwin: Plug memory leak in fallback kill() logic. Thanks [@tmm1][]!
- fruity: Handle failure to fetch dyld symbols.
- compiler: Bump ainakan-compile to 16.2.2. Dependencies' source maps are now also
  bundled -- thanks [@vfsfitvnm][]!
- compiler: Bump @types/ainakan-gum to 18.3.2, now with improved typings for
  *hexdump()*.
...
[this internal API]: https://github.com/ainakan/ainakan-core/blob/0c6737becb603871f62c775f06214b27c3e208ad/src/barebone/script.vala#L220-L257
[GDB.Client]: https://github.com/ainakan/ainakan-core/blob/0c6737becb603871f62c775f06214b27c3e208ad/src/gdb.vala#L3
[Tamarin Cable]: https://github.com/stacksmashing/tamarin-firmware
[parsing the page tables]: https://github.com/ainakan/ainakan-core/blob/0c6737becb603871f62c775f06214b27c3e208ad/src/barebone/arch-arm64/machine.vala#L38-L91
...
[memory scanner implementation]: https://github.com/ainakan/ainakan-core/tree/0c6737becb603871f62c775f06214b27c3e208ad/src/barebone/helpers
