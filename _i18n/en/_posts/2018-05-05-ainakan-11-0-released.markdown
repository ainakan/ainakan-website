...
Say you're using Ainakan's Python bindings, you'd currently do:
...
in ainakan-core, which these bindings expose to different languages:
...
That's [Vala][] code by the way, which is the language that ainakan-core is
...
{% highlight c %}
void ainakan_device_spawn (AinakanDevice * self,
    const gchar * path,
    gchar ** argv, int argv_length,
    gchar ** envp, int envp_length,
    GAsyncReadyCallback callback, gpointer user_data);
guint ainakan_device_spawn_finish (AinakanDevice * self,
    GAsyncResult * result, GError ** error);
guint ainakan_device_spawn_sync (AinakanDevice * self,
    const gchar * path,
    gchar ** argv, int argv_length,
    gchar ** envp, int envp_length,
    GError ** error);
...
As for the third, *spawn_sync()*, this is what Ainakan's Python bindings use.
...
exposed. If you look closely at the ainakan-core API you'll notice that there's
...
namespace Ainakan {
	...
	public class Device : GLib.Object {
		...
		public async uint spawn (string program,
			Ainakan.SpawnOptions? options = null)
			throws Ainakan.Error;
	...
	public class SpawnOptions : GLib.Object {
		...
		public Ainakan.Stdio stdio { get; set; }
...
{% highlight python %}
device.spawn("com.apple.mobilesafari", url="https://ainakan.re")
{% endhighlight %}
...
- ainakan-trace: fix crash when tracer scripts *send()* arbitrary data
...
So that's about it. If you didn't read about the Ainakan 10.8 release that
...
[here]: /news/2018/04/28/ainakan-10-8-released/
