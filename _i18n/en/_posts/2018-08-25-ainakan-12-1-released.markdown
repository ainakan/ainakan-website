...
Ainakan's V8 dependency was previously at 6.2.2, and has now been upgraded to
...
To start using it, just tell Ainakan to use V8, by calling *session.enable_jit()*,
and then *session.enable_debugger()*.

Or when using the CLI tools:

{% highlight sh %}
$ ainakan-trace --enable-jit --debug -f /bin/cat -i read
{% endhighlight %}
...
debugging your Ainakan scripts. That means a nifty console with auto-completion,
...
[viniciusmarangoni][] this release also packs a couple of goodies for ainakan-java,
...
[preloaded mode]: https://ainakan.re/docs/modes/#preloaded
