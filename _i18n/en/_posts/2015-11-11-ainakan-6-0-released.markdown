...
- ainakan-repl: fix display of raw buffers as returned by *Memory.readByteArray()*
- ainakan-repl: fix crash in completion on error
...
- node: Babelify the source code read by *ainakan.load()*
- node: remove *ainakan.load()* as it's now in the ainakan-load module
...
- core: provide AinakanGadget.dylib for instrumenting iOS apps without jailbreak
...
- ainakan-trace: always treat handler scripts as UTF-8
...
- ainakan-trace: improve manpage parser
...
- core: add support for running ainakan-gadget standalone
