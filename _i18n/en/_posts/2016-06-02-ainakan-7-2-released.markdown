...
Some of you may be aware that Ainakan has two JavaScript runtimes, one based
...
runtime by forcing Ainakan to favor Duktape through *Session.disable_jit()*.
...
co(function* () {
  const systemSession = yield ainakan.attach(0);
  ...
  const session = yield ainakan.attach('Twitter');
...
to a future version of Ainakan, but will at least be architecture-neutral; i.e.
you can compile to bytecode on a 64-bit x86 desktop and load it just fine in
a 32-bit iOS app on ARM.

So that's bytecode compilation through the API, but you probably want to use the [ainakan-compile](https://github.com/ainakan/ainakan-compile)
CLI tool for this instead:

{% highlight bash %}
$ npm install ainakan-compile
$ ./node_modules/.bin/ainakan-compile agent.js -o agent.bin -b
{% endhighlight %}
...
Whether you're using bytecode (*-b*) or not, ainakan-compile is highly recommended
...
- Leverage thousands of existing modules from npm, including some that are
  Ainakan-specific. For example:
  [ainakan-trace](https://github.com/nowsecure/ainakan-trace),
  [ainakan-uikit](https://github.com/nowsecure/ainakan-uikit),
  [ainakan-screenshot](https://github.com/nowsecure/ainakan-screenshot), etc.
...
- core: fix *dlsym()* deadlock in ainakan-gadget on Linux
...
- ainakan-server: fix "hang" on detach when another client is attached to the
                same process
