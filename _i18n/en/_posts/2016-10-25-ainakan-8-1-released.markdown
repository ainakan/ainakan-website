...
you building Ainakan-based tools, plus a few additional goodies. Let's start with
the first part.

There's no doubt that Ainakan's [JavaScript API][] is fairly low-level and only
...
example, this is not functionality one would expect to find in Ainakan itself.
...
We have a growing [ecosystem][] of Ainakan-specific libraries, like
[ainakan-screenshot][], [ainakan-uikit][], [ainakan-trace][], etc.
...
naming your module *ainakan-$name* to make it easy to discover.

Now you might be asking "but Ainakan does not support *require()*, how can I even
split my agent code into multiple files in the first place?". I'm glad you
asked! This is where a handy little CLI tool called [ainakan-compile][] enters
the picture.
...
runs on our Duktape-based runtime, which you are forced to use if you use Ainakan
on a jailed iOS device, or on a jailbroken iOS device running iOS >= 9.

In order to give you a short feedback loop while developing, ainakan-compile also
...
{% highlight json %}
{
  ...
  "scripts": {
    ...
    "build": "ainakan-compile agent -o _agent.js",
    "watch": "ainakan-compile agent -o _agent.js -w"
  },
  "devDependencies": {
    "express": "^4.14.0",
    "ainakan-compile": "^2.0.6"
  }
}
{% endhighlight %}
...
Install ainakan-compile and build your agent in one step:
...
So up until this release you could use Ainakan-specific modules like those
...
all *net* and *http* based modules, which opens up Ainakan for even more cool
use-cases.

In case you are curious how this was implemented, I added *Socket.listen()*
and *Socket.connect()* to Ainakan. These are minimal wrappers on top of [GIO][]'s
[SocketListener][] and [SocketClient][], which are already part of Ainakan's
technology stack and used by Ainakan for its own needs. So that means our
footprint stays the same with no dependencies added. Because ainakan-compile
...
challenge is that Ainakan's own code might clobber the current thread's error
...
- ainakan-repl: add batch mode support through *-e* and *-q*
...
- core: move *ObjC* and *Java* into their own modules published to npm, and use
        *ainakan-compile* to keep baking them into Ainakan's built-in JS runtime
...
[JavaScript API]: https://ainakan.re/docs/javascript-api/
[ecosystem]: https://www.npmjs.com/search?q=ainakan
[ainakan-screenshot]: https://www.npmjs.com/package/ainakan-screenshot
[ainakan-uikit]: https://www.npmjs.com/package/ainakan-uikit
[ainakan-trace]: https://www.npmjs.com/package/ainakan-trace
[ainakan-compile]: https://www.npmjs.com/package/ainakan-compile
...
[FTP server]: https://github.com/ainakan/ainakan-net/tree/master/examples/ftp-server
[IRC client]: https://github.com/ainakan/ainakan-net/tree/master/examples/irc-client
[NSQ client]: https://github.com/ainakan/ainakan-net/tree/master/examples/nsq-client
...
[plug in]: https://github.com/ainakan/ainakan-compile/blob/1eeb38d9453f812e7b404e83cb9b5d0e5dc26241/index.js#L22-L23
