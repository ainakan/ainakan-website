...
While Ainakan's instrumentation core, Gum, is written in C and can be used from C,
...
portable option for use in modules such as [ainakan-java-bridge][].
...
only adds ~100 kB of footprint to Ainakan.
...
headers in [ainakan-gum/bindings/gumjs/runtime/cmodule][] to get an overview.
Drop function names into an Internet search engine to look up docs for the
non-Ainakan APIs such as GLib's.
...
- The *ainakan-inject* tool now supports *spawn()*. Kudos to [@hunterli][] for
...
- The *ainakan-inject* tool now supports *-D/--device* for working with non-local
  devices.
...
- Eternalized agents are now kept around when the last session gets closed,
  ...
  ainakan-server, sticks around. This means that additional copies of ainakan-agent
...
- Massively improved fault-tolerance on iOS: our launchd agent now kills pending
  processes when unloaded. This means that ainakan-server dying won't leave
...
- Ainakan no longer crashes in Android processes without a vDSO. (Regression
...
- Full-featured iOS lockdown integration and unified devices, so Ainakan-based
...
- Ainakan is finally able to detect recent iOS devices on Windows.
- Bug in V8 was tracked down and fix backported from upstream. Kudos to
  [@mrmacete][] for tracking this one down!

### Changes in 12.7.13

- Better handling of structs and unions in *ainakan-objc-bridge*. Thanks
  [@gebing][]!
...
- Ainakan now ensures code is readable before accessing it on Android >= 10.
  ...
  spawn() – works, and starting ainakan-server doesn't crash *system_server* due
...
- The *ainakan-gadget-ios* meta-package comes with type definitions so it can be
...
- Ainakan now communicates directly with the remote iOS/Android agent when
  possible. We achieve this by establishing a fresh TCP connection to the
  ainakan-server and having its file-descriptor passed to the agent. This means
  ainakan-server can remove itself from the data path, improving performance and
  reliability.
- Clients connecting to ainakan-server to spawn() a process, but getting
...
- Our TCP sockets now have *TCP_NODELAY* set, and we also support communicating
  with a remote ainakan-server or ainakan-gadget using UNIX sockets in addition to
  TCP.
...
- Ainakan now supports both cleartext and encrypted iOS lockdown channels,
...
- Our iOS package scripts launch daemon logic is now compatible with checkra1n,
  so ainakan-server won't have to be started/stopped manually.
...
- Ainakan now makes parent file-descriptors available to child processes when
...
- NativeFunction got a brand new option `traps: 'all'` to allow calls to be
  stalked even when Ainakan's own memory range is marked as excluded.
- Thread enumeration on Linux is finally working when using Yama. Thanks
  [Jon Wilson][]!


[ainakan-java-bridge]: https://github.com/ainakan/ainakan-java-bridge
[TinyCC]: https://bellard.org/tcc/
[ainakan-compile]: https://github.com/oleavr/ainakan-agent-example
...
[ainakan-gum/bindings/gumjs/runtime/cmodule]: https://github.com/ainakan/ainakan-gum/tree/master/bindings/gumjs/runtime/cmodule
