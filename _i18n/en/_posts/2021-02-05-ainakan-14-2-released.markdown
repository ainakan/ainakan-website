...
Ainakan has supported Android for quite a while, but one particular feature has
kept getting requested (mostly) by users who thought they were looking at a bug.
The conversation usually started something like: “I'm using Ainakan inside the
...
In a Ainakan context, however, we're usually talking about a VirtualBox-based
...
in Ainakan, but thinking about it always made my head hurt a little. It did feel
...
the following ainakan-core API, where the Python flavor looks like this:

{% highlight python %}
session = device.attach(target)
{% endhighlight %}

(Or *ainakan.attach()* if your code only deals with the local system.)
...
The way Ainakan's [Java bridge][] replaces Java methods on Android has up until
...
who started porting Ainakan to S390x. Our CModule implementation relies on TinyCC
...
Also, as of ainakan-tools 9.2, the REPL's `-C` switch also supports this, making
...
And on that note we now have such a tool in ainakan-tools:

{% highlight sh %}
$ mkdir pewpew
$ cd pewpew
$ ainakan-create cmodule
Created ./meson.build
Created ./pewpew.c
Created ./.gitignore
...
- Inject CModule using the REPL: ainakan Calculator -C ./build/pewpew.dylib
...
$ ainakan Calculator -C ./build/pewpew.dylib
…
init()
[Local::Calculator]->
{% endhighlight %}
...
- Add --disable-preload/-P to ainakan-server. Useful in case of OS compatibility
  issues where Ainakan crashes certain OS processes when attaching to them.
...
- Fix direct transport regression on i/macOS, introduced by GLib upgrade where
  ...
  of this regression is that ainakan-server gets killed by Jetsam.
...
- Fix ainakan-java-bridge crash on systems where printf() renders %p without
  “0x” prefix.
...
- Fix GLib SO_NOSIGPIPE regression on i/macOS. This would typically result in
  ainakan-server dying due to SIGPIPE. Thanks [@mrmacete][]!
...
- Generate ainakan-core GIR by default. Thanks [@meme][]!
- Fix regressions on Linux/MIPS.

### Changes in 14.2.10

- Improve ainakan-inject to support bidirectional stdio.
- Add support for Termux in ainakan-python: `pip install ainakan-tools` now works.

### Changes in 14.2.11

- Improve ainakan-inject to support raw terminal mode.
...
- Fix ainakan-inject when process is not attached to a terminal. Thanks
  [@muhzii][]!
...
- Make ainakan-inject stdin communication optional. Thanks [@muhzii][]!
...
- Fix hang when target is unable to load ainakan-agent.
...
[Java bridge]: https://github.com/ainakan/ainakan-java-bridge
[@muhzii]: https://github.com/muhzii
[@mephi42]: https://github.com/mephi42
[dynamic linker]: https://github.com/ainakan/ainakan-gum/blob/master/gum/backend-darwin/gumdarwinmapper.h
