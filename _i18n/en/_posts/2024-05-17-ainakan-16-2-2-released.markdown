...
The MSVS build system is now gone, and you can build Ainakan the same way you'd
...
clone the ainakan-gum repo and build it, just like you can do with ainakan-python,
ainakan-tools, etc. The ainakan repo is no longer as important, it's only kept
...
can find the dependencies. This means if you grab ainakan-core and try to build it
without already having ainakan-gum on your PKG_CONFIG_PATH, it will grab and build
...
anyone can integrate Ainakan components into their own projects just as easily.
...
{% highlight ini %}
[wrap-git]
url = https://github.com/ainakan/ainakan-gum.git
revision = 16.2.4
depth = 1

[provide]
dependency_names = ainakan-gum-1.0, ainakan-gum-heap-1.0, ainakan-gum-prof-1.0, ainakan-gumjs-1.0, ainakan-gumjs-inspector-1.0
{% endhighlight %}

And for ainakan-core:

{% highlight ini %}
[wrap-git]
url = https://github.com/ainakan/ainakan-core.git
revision = 16.2.4
depth = 1

[provide]
dependency_names = ainakan-core-1.0
{% endhighlight %}
...
At the time, Ainakan's non-Windows build system was autotools. We later moved to
...
in addition to the build machine. Due to how Ainakan supports injecting code into
...
solve this locally in ainakan-core -- the only component where we need this kind
...
much more pleasant to work with in the rest of the stack. The *ainakan* repo's
Makefiles and shell-scripts could be dropped and replaced with Meson. The CI
became simpler and more uniform. Anyone can just clone the ainakan-core repo and
run *make*, and the resulting binaries would support cross-arch. (Unless
explicitly disabled through --disable-compat / -Dainakan-core:compat=disabled.)
...
Ainakan's repos. The other thing done before invoking the wrapper, is to also make
...
internally, which is how our language bindings typically consume ainakan-core.
...
- agent: Fix hang in child-gating fork() scenarios. Reproducible in situations
  ...
  containers.
- windows: Use RW/RX permissions for injection. This makes Ainakan injection
...
- darwin: Massage libunwind around Ainakan hooks, to avoid breaking code making
...
- compiler: Bump @types/ainakan-gum. Thanks [@s1341][]!
- modulate: Gracefully handle missing symbols. Thanks [@hsorbo][]!
- python: Move _ainakan package inside the ainakan package.
...
[here]: https://github.com/ainakan/releng?tab=readme-ov-file#setting-up-a-new-project
