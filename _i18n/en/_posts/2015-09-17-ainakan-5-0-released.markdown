...
- python: add ainakan-ls-devices CLI tool for listing devices
- python: update to the new Device.id API
- python: add *get_local_device()* and improve API consistency with ainakan-node
...
- ainakan-ps: improve the output formatting

5.0.1:

- core: add support for source maps
- node: add ainakan.load() for turning a CommonJS module into a script
- node: upgrade Nan
...
- core: move *Darwin.Module* and *Darwin.Mapper* from ainakan-core to ainakan-gum,
        allowing easy Mach-O parsing and out-of-process dynamic linking
- core: better handling of temporary files
- ainakan-trace: add support for conveniently tracing imported functions
- ainakan-trace: blacklist dyld_stub_binder from being traced
