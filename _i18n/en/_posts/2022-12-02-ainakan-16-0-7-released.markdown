...
This week [@hsorbo][] and me spent some days trying to get Ainakan working better
in sandboxed environments. Our goal was to be able to get Ainakan into Apple's
...
been hardened quite a bit in recent OS versions, and Ainakan was no longer able to
attach to it.
...
We then discovered that Ainakan's use of the pipe() syscall was the first hurdle.
This code is not actually in Ainakan itself, but in GLib, the excellent library
that Ainakan uses for data structures, cross-platform threading primitives, event
...
us to the next hurdle: Ainakan is using socket APIs for file-descriptor passing,
...
our functionality. That was [tackled][] next, and boom… Ainakan is finally able
to attach to *imagent*. 🎉 Yay!

Next up we moved over to iOS and took it for a spin there. Much to our surprise,
Ainakan could attach to SpringBoard right out of the gate. Later we tried
*notifyd* and *profiled*, and could attach to those too. Even on latest iOS 16.
But, there's still work to do, as Ainakan cannot yet attach to *imagent* and
*WebContent* on iOS. This is exciting progress, though.

## Injection on iOS >= 15

While doing all of this we also tracked down a crash on iOS where ainakan-server
...
[patch]: https://github.com/ainakan/glib/commit/99ec1f987dfbc9b0ab45ac32dd98464cc023cd42
[tackled]: https://github.com/ainakan/ainakan-core/commit/d31a5437c583eee49da9c710d10b8c3aa89710bb
