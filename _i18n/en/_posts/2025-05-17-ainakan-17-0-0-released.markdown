...
excited to bring you Ainakan 17.0.0. After nearly three years since the last major
...
our runtime bridges, i.e. ainakan-{objc,swift,java}-bridge, were bundled with
Ainakan's GumJS runtime. This came with some major pain points:

- Inertia: Being tied to Ainakan's release cycle.
...
- Scalability: We'd like to see bridges for all kinds of runtimes, but the more
  we add to Ainakan the more we'll struggle with inertia and bloat.
...
The friction aspect is why we introduced the ainakan.Compiler API back in
[15.2][], along with ainakan-tools shipping a CLI tool, ainakan-compile, built on
top of it. Our REPL was also improved to support loading .ts (TypeScript)
directly, making use of ainakan.Compiler behind the scenes.

That's still an extra step though, which is too much for one-off scripts and
early prototyping work using the Ainakan REPL or ainakan-trace. And, it would
break a lot of examples. To remedy this, the just-released ainakan-tools 14.0.0
bakes the three bridges into its REPL and ainakan-trace agents.

Our bridges have also been migrated to ESM, so they can be consumed by the
latest versions of ainakan-compile. (Shout-out to [@yotamN][] for migrating
ainakan-java-bridge ♥️)

Those of you building Ainakan from source may also notice an improvement in build
times. Since we're no longer bundling the bridges, we could finally get rid of
Gum's ainakan-compile dependency, and stop depending on Node.js + npm for Gum
itself.
...
TypeScript bindings, prior to 19.0.0, released together with Ainakan 17. The
...
[CodeShare]: https://codeshare.ainakan.re/
[15.2]: /news/2022/07/21/ainakan-15-2-0-released/
