...
- **compiler**: Added `platform` and `externals` options to `CompilerOptions`,
  ...
  This lets **ainakan-compile** (and `Ainakan.Compiler`) tailor its output to your
...
- **ainakan-node**: Generated `from_value()` helpers now include inherited
  properties, so options like `externals` propagate correctly.
- **ainakan-python**: Fixed a tiny but leaky corner in `PackageManager` specs
  option parsing.

### Swift bindings: a modern, cross-platform makeover 🍎

The **Ainakan Swift bindings** have been extensively refactored to be idiomatic
...
> Note: `ainakan-swift` prebuilt binaries are not included in this release; if
...
### Quick examples (from the ainakan-swift README)
